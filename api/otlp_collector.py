...
class OTLPCollector:
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    row["container_name"] or agent_name

                    # Build agent URL - use production URLs if available
                    # Check for environment URL first (for production agents)
                    clean_name = agent_id.upper().replace("-", "_")
                    env_url = os.getenv(f"AGENT_{clean_name}_URL", "")
                    if env_url:
                        agent_url = env_url.rstrip("/")
                    else:
                        # Fallback to local Docker access
                ...
                if not configs:
                    ...
                    if total_agents_count > 0:
                        logger.error(
                            f"CRITICAL: All {total_agents_count} known agents are now unreachable! "
                        ...
                        if rows:
                            logger.warning(
                                f"FALLBACK: Using {len(rows)} stale agent configurations. "
                            ...
                            logger.error(
                                "CRITICAL: No fallback agents available. Monitoring is down!"
                            )
                    else:
    ...
    async def start(self):
        ...
        logger.info("Started OTLP collector with dynamic agent discovery")
    ...
    async def collect_agent_loop(self, config: dict[str, str]):
        ...
        logger.info(
            f"Starting OTLP collection for {agent_name} (ID: {agent_id}) at {agent_url} every {interval}s"
        )
    ...
    async def store_otlp_data(
        self, agent_name: str, metrics: dict | None, traces: dict | None, logs: dict | None
    ):
                        ...
                        try:
                            ...
                            """,
                                ...
                                datetime.fromtimestamp(
                                    float(point.get("timeUnixNano", 0)) / 1e9, tz=UTC
                                ),
                            )
                            metric_count += 1
                        except Exception as e:
            ...
            for scope_span in resource_span.get("scopeSpans", []):
                ...
                for span in spans:
                    ...
                    try:
                        ...
                        end_time = datetime.fromtimestamp(end_time_nano / 1e9, tz=UTC)

                        # Debug logging for first few traces
                        if trace_count < 3:
                            logger.info(
                                f"DEBUG: Processing trace {span.get('traceId', 'unknown')[:16]}... "
                        ...
                        if trace_count < 3:
                            logger.info(
                                f"DEBUG: Insert result: {result} for trace {span.get('traceId', 'unknown')[:16]}..."
                            )
            ...
            for scope_log in resource_log.get("scopeLogs", []):
                for log_record in scope_log.get("logRecords", []):
                    ...
                    """,
                        agent_name,
                        datetime.fromtimestamp(
                            float(log_record.get("timeUnixNano", 0)) / 1e9, tz=UTC
                        ),
                        severity,
