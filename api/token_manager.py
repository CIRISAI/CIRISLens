...
class TokenManager:
    ...
    def _load_metadata(self):
        ...
        if self.tokens_metadata_file.exists():
            self.metadata = json.loads(self.tokens_metadata_file.read_text())
        else:
            self.metadata = {}

    def _save_metadata(self):
        """Save token metadata"""
        self.tokens_metadata_file.write_text(json.dumps(self.metadata, indent=2, default=str))
        ...
        for key in os.environ:
            if key.startswith("AGENT_") and key.endswith("_TOKEN"):
                ...
                if agent_name in self.metadata:
                    ...
                    agent_info["token_hash"] = (
                        self.metadata[agent_name].get("token_hash", "")[:8] + "..."
                    )
    ...
    async def set_agent_token(self, agent_name: str, token: str, url: str, updated_by: str) -> bool:
        ...
        try:
            ...
            if self.env_file_path.exists():
                async with aiofiles.open(self.env_file_path) as f:
