"""
CIRIS Accord API V2 - Rust-powered trace ingestion
...
Note: "Accord" replaces the former "Covenant" naming.
Backward-compatible aliases are provided via covenant_api_v2.py.
"""
...
async def store_production_trace(
    ...
    """Store accepted trace in accord_traces table (currently covenant_traces).
    ...
    """,
        ...
        json.dumps(metadata.get('models_used')) if metadata.get('models_used') else None,  # $44 - JSONB
        signature,                                        # $45
        signature_key_id,                                 # $46
...
async def receive_accord_events(request: Request) -> dict[str, Any]:
    ...
    logger.info("EVENTS_REQUEST_RECEIVED: keys=%s consent_timestamp=%s batch_timestamp=%s",
                list(raw_data.keys()),
                raw_data.get('consent_timestamp', 'MISSING'),
                raw_data.get('batch_timestamp', 'MISSING'))
    ...
    try:
        validated_request = AccordEventsRequest.model_validate(raw_data)
    except Exception as e:
        error_str = str(e)
    ...
    logger.info(
        "Accord events batch: received=%d accepted=%d rejected=%d",
...
# /repository/traces + /repository/traces/{trace_id} live in accord_api.py
# (v1 router), which registers first in main.py and wins the route. The v2
# duplicates that used to live here were dead code; their replacement —
# pass-through of CIRISPersist v0.5.0 §A/§B primitives — is in accord_api.py.
