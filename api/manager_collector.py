...
class ManagerCollector:
    ...
    async def start(self):
        ...
        try:
            ...
            if not managers:
                logger.warning(
                    "ManagerCollector: No enabled managers found - no collection tasks started"
                )
            else:
                for manager in managers:
                    logger.info(
                        f"ManagerCollector: Starting collection task for {manager.get('name', 'unknown')}"
                    )
                    task = asyncio.create_task(self.collect_manager_loop(manager))
                    self.tasks.append(task)

                logger.info(
                    f"ManagerCollector: Started {len(self.tasks)} collection tasks successfully"
                )
    ...
    async def get_enabled_managers(self) -> list[dict]:
        ...
        async with self.pool.acquire() as conn:
            rows = await conn.fetch("SELECT * FROM managers WHERE status = 'online'")
    ...
    async def collect_manager_loop(self, manager: dict):
        ...
        logger.info(
            f"ManagerCollector: Collection loop STARTED for {manager_name} ({manager_url}) every {interval}s"
        )
        ...
        while self.running:
            try:
                ...
                if iteration <= 3:
                    logger.info(
                        f"ManagerCollector: Starting collection iteration {iteration} for {manager_name}"
                    )
                await self.collect_from_manager(manager)
                if iteration <= 3:
                    logger.info(
                        f"ManagerCollector: Completed collection iteration {iteration} for {manager_name}"
                    )
            except Exception as e:
                logger.error(
                    f"COLLECTION FAILURE for {manager_name} (iteration {iteration}): {e}",
                    exc_info=True,
                )
    ...
    async def collect_from_manager(self, manager: dict):
        ...
        async with httpx.AsyncClient(timeout=10.0, verify=SSL_VERIFY) as client:
            ...
            # Collect agents list
            try:
                agents_response = await client.get(f"{manager_url}/agents", headers=headers)
        ...
        if status_data and not agents_data:
            logger.error(
                f"CRITICAL: Manager {manager_name} is running but returned NO agents! "
    ...
    async def store_manager_telemetry(
        self, manager_id: str, status_data: dict | None, agents_data: list[dict]
    ):
    ...
    async def remove_manager(self, manager_id: int):
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("UPDATE managers SET enabled = false WHERE id = $1", manager_id)
